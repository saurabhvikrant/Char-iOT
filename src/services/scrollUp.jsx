...
export default class ScrollUp extends React.Component {
	...
	componentDidMount() {
		...
		window.addEventListener("wheel", this.stopScrolling, detectPassiveEvents.hasSupport
			? {
				passive: true
			}
			: false);
		window.addEventListener("touchstart", this.stopScrolling, detectPassiveEvents.hasSupport
			? {
				passive: true
			}
			: false);
	...
	render() {
		...
		let element = <div style={propStyle} onClick={this.handleClick}>
			{this.props.children}
		</div>;

		let style = objectAssign({}, ScrollUp.defaultProps.style);
		style = objectAssign(style, propStyle);
		style.opacity = (this.state.show
			? 1
			: 0);
		style.visibility = (this.state.show
			? 'visible'
			: 'hidden');
